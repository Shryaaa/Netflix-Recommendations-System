...
for new_feature in new_features:
    selected_data.loc[:, new_feature] = selected_data[new_feature].apply(prepare_data)
...
def getvalue():
    ...
    for moviename in movienames:
        ...
        for language in languages:
            df = pd.concat([recommended_movies[recommended_movies['Languages'].str.count(language) > 0], df], ignore_index=True)
